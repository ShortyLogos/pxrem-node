...
export default class LineWorker {
  ...
  async convert() {
    ...
    for await (const match of this.matches) {
      const unparsedSourceValue = match.substring(
        0,
        match.length - this.converter.sourceLabel.length
      );

      const sourceValue = this.converter.hasFloating
        ? parseFloat(unparsedSourceValue)
        : parseInt(unparsedSourceValue);
